...
int tst_coll_alltoall_run (const struct tst_env * env)
{
  ...
  tst_coll_alltoall_setarray (env->type, env->values_num, send_buffer, comm_rank, comm_size);
  ...
  tst_coll_alltoall_checkarray (env->type, env->values_num, recv_buffer, comm_rank, comm_size);
