...
int tst_coll_reduce_in_place_max_init (struct tst_env * env)
{
  int comm_rank;
  MPI_Comm comm;
...
int tst_coll_reduce_in_place_max_run (struct tst_env * env)
{
  int comm_rank;
  int comm_size;
  MPI_Comm comm;
  ...
  if (ROOT == comm_rank) {
     ...
     MPI_CHECK (MPI_Reduce (MPI_IN_PLACE, env->recv_buffer, env->values_num, type, MPI_MAX, ROOT, comm));
  } else {
...
int tst_coll_reduce_in_place_max_cleanup (struct tst_env * env)
{
  int comm_rank;
  MPI_Comm comm;
