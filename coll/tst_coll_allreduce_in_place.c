...
int tst_coll_allreduce_in_place_init (const struct tst_env * env)
{
...
int tst_coll_allreduce_in_place_run (const struct tst_env * env)
{
  int comm_rank;
  int comm_size;
  MPI_Comm comm;
  ...
  MPI_CHECK (MPI_Allreduce (MPI_IN_PLACE, recv_buffer, env->values_num, type, MPI_MAX, comm));
...
int tst_coll_allreduce_in_place_cleanup (const struct tst_env * env)
{
