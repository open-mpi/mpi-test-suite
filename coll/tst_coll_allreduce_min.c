...
int tst_coll_allreduce_min_init (struct tst_env * env)
{
...
int tst_coll_allreduce_min_run (struct tst_env * env)
{
  int comm_rank;
  int comm_size;
  MPI_Comm comm;
  ...
  MPI_CHECK (MPI_Allreduce (env->send_buffer, env->recv_buffer, env->values_num, type, MPI_MIN, comm));
  tst_test_checkstandardarray (env, env->recv_buffer, 0);

  return 0;
}

int tst_coll_allreduce_min_cleanup (struct tst_env * env)
{
