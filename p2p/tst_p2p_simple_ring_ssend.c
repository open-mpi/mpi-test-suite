...
int tst_p2p_simple_ring_ssend_init (struct tst_env * env)
{
  int comm_rank;
  MPI_Comm comm;
...
int tst_p2p_simple_ring_ssend_run (struct tst_env * env)
{
  int comm_size;
  int comm_rank;
  int send_to;
  ...
  if (comm_rank == 0)
    {
      MPI_CHECK (MPI_Ssend (env->send_buffer, env->values_num, type, send_to, env->tag, comm));
...
int tst_p2p_simple_ring_ssend_cleanup (struct tst_env * env)
{
