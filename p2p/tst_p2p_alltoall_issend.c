...
int tst_p2p_alltoall_issend_init (const struct tst_env * env)
{
  MPI_Comm comm;
  int comm_rank;
  int comm_size;
...
int tst_p2p_alltoall_issend_run (const struct tst_env * env)
{
  int comm_size;
  int comm_rank;
  int rank;
  ...
  for (rank = 0; rank < comm_size; rank++)
    {
      ...
      MPI_CHECK (MPI_Issend (send_buffer, env->values_num, type,
                             rank, 4711, comm,
                             &(req_buffer[2*rank + 0])));
...
int tst_p2p_alltoall_issend_cleanup (const struct tst_env * env)
{
  MPI_Comm comm;
  int i;
  int comm_size;
