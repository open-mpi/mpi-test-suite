...
int tst_p2p_simple_ring_isend_init (const struct tst_env * env)
{
  int comm_rank;
  MPI_Comm comm;
...
int tst_p2p_simple_ring_isend_run (const struct tst_env * env)
{
  int comm_size;
  int comm_rank;
  int send_to;
...
int tst_p2p_simple_ring_isend_cleanup (const struct tst_env * env)
{
  free (send_buffer);
  free (recv_buffer);
  return 0;
