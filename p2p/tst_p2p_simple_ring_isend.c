...
int tst_p2p_simple_ring_isend_init (struct tst_env * env)
{
  int comm_rank;
  MPI_Comm comm;
...
int tst_p2p_simple_ring_isend_run (struct tst_env * env)
{
  int comm_size;
  int comm_rank;
  int send_to;
  ...
  MPI_CHECK (MPI_Isend (env->send_buffer, env->values_num, type, send_to, env->tag, comm, &requests[0]));
...
int tst_p2p_simple_ring_isend_cleanup (struct tst_env * env)
{
