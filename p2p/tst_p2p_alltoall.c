...
int tst_p2p_alltoall_run (struct tst_env * env)
{
  int comm_size;
  int comm_rank;
  int rank;
  ...
  for (rank = 0; rank < comm_size; rank++)
    {
      ...
      MPI_CHECK (MPI_Isend (env->send_buffer, env->values_num, type,
                            rank, env->tag, comm,
  ...
  for (rank = 0; rank < comm_size; rank++)
    {
      ...
      tst_test_checkstandardarray (env, env->recv_buffer_array[rank], comm_size - rank - 1);
    }
  return 0;
}

int tst_p2p_alltoall_cleanup (struct tst_env * env)
{
  MPI_Comm comm;
  int i;
  int comm_size;
