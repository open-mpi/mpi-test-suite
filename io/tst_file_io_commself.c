...
int tst_file_io_commself_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_io_commself_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  for(i = 0; i<comm_size; i++)
    {
      if(i == comm_rank)
        {
          MPI_CHECK ( MPI_File_open(MPI_COMM_SELF, file_name,MPI_MODE_CREATE |MPI_MODE_WRONLY, MPI_INFO_NULL, &file));
  ...
  if(off_position != env->values_num * (comm_rank+1))
    ERROR (EINVAL, "Error in offset position");
  if(off_view != 0)
...
int tst_file_io_commself_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
