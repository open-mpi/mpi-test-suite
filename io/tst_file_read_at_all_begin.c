/*
 * File: tst_file_read_at_all_begin.c
 * Functionality:
 *  test the Function MPI_File_read_at_all_begin
...
int tst_file_read_at_all_begin_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_read_at_all_begin_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
  ...
  MPI_CHECK (MPI_File_read_at_all_begin(file, comm_rank*env->values_num,
                                        read_buffer,env->values_num, type));
  MPI_CHECK (MPI_File_read_at_all_end(file, read_buffer, &stat));
...
int tst_file_read_at_all_begin_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
