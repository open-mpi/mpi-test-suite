...
int tst_file_read_ordered_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_read_ordered_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
  ...
  MPI_CHECK (MPI_File_read_ordered(file,read_buffer,env->values_num, type,&stat));
...
int tst_file_read_ordered_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
