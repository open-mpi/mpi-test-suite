 ...
 * File: tst_iread.c
 * Functionality:
 *  test the Function MPI_File_iread
...
int tst_file_iread_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_iread_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
  ...
  MPI_CHECK(MPI_File_iread(file,read_buffer,env->values_num, type,&req));
...
int tst_file_iread_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
