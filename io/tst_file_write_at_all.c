...
int tst_file_write_at_all_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_write_at_all_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  MPI_CHECK (MPI_File_write_at_all(file, comm_rank*env->values_num,
  ...
  if(off_position != 0)
    ERROR (EINVAL, "Error in position after MPI_File_write_at_all");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_write_at_all");
...
int tst_file_write_at_all_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
