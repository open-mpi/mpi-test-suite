...
int tst_file_read_convert_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_read_convert_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  MPI_CHECK ( MPI_File_seek(file, (-1)*(comm_rank + 1)*env->values_num, MPI_SEEK_END));
  ...
  if(off_position != env->values_num * (comm_size - comm_rank))
    ERROR (EINVAL, "Error in position after MPI_File_read");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_read");
  ...
  tst_test_checkstandardarray(env, read_buffer, (comm_size - comm_rank - 1));
...
int tst_file_read_convert_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
