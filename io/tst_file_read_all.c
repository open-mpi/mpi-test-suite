/*
 * File: tst_file_read_all.c
 * Functionality:
 *  test the Function MPI_File_read
...
int tst_file_read_all_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_read_all_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
  ...
  if(off_position != env->values_num * (comm_rank+1))
    ERROR (EINVAL, "Error in position after MPI_File_read_all");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view point after MPI_File_read_all");
...
int tst_file_read_all_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
