 ...
 * File: tst_iread_at.c
 * Functionality:
 *  test the Function MPI_File_iread_at
...
int tst_file_iread_at_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_iread_at_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
  ...
  MPI_CHECK (MPI_File_iread_at(file, comm_rank*env->values_num,
                               read_buffer,env->values_num, type,&req));
...
int tst_file_iread_at_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
