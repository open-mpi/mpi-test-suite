/*
 * File: tst_file_read_at.c
 * Functionality:
 *  test the Function MPI_File_read_at
...
int tst_file_read_at_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_size;
...
int tst_file_read_at_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
  ...
  MPI_Offset off_view,off_position, off_position_byte;
  ...
  MPI_CHECK (MPI_File_read_at(file, comm_rank*env->values_num,
  ...
  if(off_position_byte != 0)
    ERROR (EINVAL, "Error in byte_offset");
...
int tst_file_read_at_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
