/*
 * File: tst_file_write_shared.c
 *
 * Functionality:
 *  Check the function MPI_File_write_shared
 *
...
int tst_file_write_shared_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_write_shared_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  for(i = 0; i < comm_size; i++)
    {
      ...
      if(comm_rank == i)
        {
          MPI_CHECK (MPI_File_write_shared(file, write_buffer,env->values_num, type,&stat));
  ...
  if(off_position != env->values_num * (comm_rank+1))
    ERROR (EINVAL, "Error in position after MPI_File_write_shared");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_write_shared");
...
int tst_file_write_shared_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
