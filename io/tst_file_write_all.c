...
int tst_file_write_all_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  if(off_position != env->values_num * (comm_rank + 1) )
    ERROR (EINVAL, "Error in position after MPI_File_write_all");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_write_all");
...
int tst_file_write_all_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
