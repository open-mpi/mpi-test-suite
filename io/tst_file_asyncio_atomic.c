/*
 * File: tst_file_asyncio_atomic.c
 *
 * Functionality:
 *  Check the function atomic mode
 + with MPI_Set_atomicity und MPI_file_Iwrite_at
 *
...
int tst_file_asyncio_atomic_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  MPI_Comm comm;
...
int tst_file_asyncio_atomic_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  ...
  MPI_CHECK ( MPI_File_iwrite_at(file, env->values_num * comm_rank, write_buffer,
                                 env->values_num, type, &(req[0])));
  MPI_CHECK ( MPI_File_get_position(file, &position1));
  MPI_CHECK ( MPI_Barrier(comm));
  MPI_CHECK ( MPI_File_iread_at(file, env->values_num * read_from, read_buffer,
                                env->values_num, type, &(req[1])));
  MPI_CHECK ( MPI_File_get_position(file, &position2));
  MPI_CHECK ( MPI_Waitall(2, req, stat));
  MPI_CHECK ( MPI_File_get_view(file, &off_view, &datatype_check, &filetype_check, datarep));

  MPI_CHECK ( MPI_File_close(&file));
  if(position1 != 0)
    ERROR (EINVAL, "Error in position after MPI_File_iwrite_at");
  if(position2 != 0)
    ERROR (EINVAL, "Error in position after MPI_File_iwrite_at");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_iwrite_at");
  if(tst_type_compare(datatype_check, type) != TST_SUCESS)
    ERROR (EINVAL, "Error in datatype after MPI_File_iwrite_at");
  if(tst_type_compare(filetype_check , type) != TST_SUCESS)
    ERROR (EINVAL, "Error in filetype after MPI_File_iwrite_at");
...
int tst_file_asyncio_atomic_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
