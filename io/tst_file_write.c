...
int tst_file_write_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_write_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  if (file == MPI_FILE_NULL) {
    ERROR (EINVAL, "MPI_File_open failed: Got FILE_NULL.");
    MPI_Finalize();
    exit(1);
  }
...
int tst_file_write_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
