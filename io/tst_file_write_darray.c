/*
 * File: tst_file_write_darray.c
 *
 * Functionality:
 *  Check the new data type MPI_darray
 + with MPI_file_write
 *
...
int tst_file_write_darray_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  if(off_position != env->values_num )
    ERROR (EINVAL, "Error in position after MPI_File_write with the darray");
  if(off_view != 0)
     ERROR (EINVAL, "Error in view position after MPI_File_write with the darray");
  if(tst_type_compare(datatype_check, type) != TST_SUCESS)
    ERROR (EINVAL, "Error in datatype");
  if(tst_type_compare(filetype_check, filetype) != TST_SUCESS)
...
int tst_file_write_darray_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
