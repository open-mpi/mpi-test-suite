/*
 * File: tst_file_write_ordered.c
 *
 * Functionality:
 *  Check the function MPI_File_write_ordered.c
 *
...
int tst_file_write_ordered_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_write_ordered_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  if(off_position != 0)
    ERROR (EINVAL, "Error in position after MPI_File_write_ordered");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_write_ordered");
...
int tst_file_write_ordered_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
