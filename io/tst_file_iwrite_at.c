/*
 * File: tst_file_iwrite_at.c
 * Functionality:
 *  test the Function MPI_File_iwrite_at
...
int tst_file_iwrite_at_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_iwrite_at_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  MPI_Offset off_position, off_view,file_size;
  ...
  MPI_CHECK(MPI_File_iwrite_at(file, comm_rank*env->values_num,
                               write_buffer,env->values_num, type,&req));
...
int tst_file_iwrite_at_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
