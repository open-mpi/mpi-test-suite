/*
 * File: tst_file_write_ordered_begin.c
 *
 * Functionality:
 *  Check the function MPI_File_write_ordered_begin.c
...
int tst_file_write_ordered_begin_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_write_ordered_begin_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  MPI_CHECK (MPI_File_write_ordered_begin(file, write_buffer,env->values_num, type));
  MPI_CHECK (MPI_File_write_ordered_end(file, write_buffer,&stat));
  ...
  if(off_position != 0)
    ERROR (EINVAL, "Error in position after MPI_File_write_ordered_begin");
  if(off_view != 0)
    ERROR (EINVAL, "Error in view position after MPI_File_write_ordered_begin");
...
int tst_file_write_ordered_begin_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
