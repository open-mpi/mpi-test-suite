...
int tst_file_set_size_init (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
...
int tst_file_set_size_run (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  int comm_rank;
  int comm_size;
  ...
  MPI_Offset check_file_size_before, check_file_size_after;
  ...
  MPI_CHECK (MPI_File_set_size(file, file_size));
...
int tst_file_set_size_cleanup (struct tst_env * env)
{
#ifdef HAVE_MPI2_IO
  MPI_Comm comm;
  int comm_rank;
